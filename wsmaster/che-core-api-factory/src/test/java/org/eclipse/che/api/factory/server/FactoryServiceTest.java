...
public class FactoryServiceTest {
    ...
    public void shouldUpdateFactory() throws Exception {
        ...
        when(factoryManager.updateFactory(any(FactoryDto.class))).thenReturn(update);
        when(factoryManager.getFactoryImages(existed.getId())).thenReturn(emptySet());
        doNothing().when(createValidator).validateOnCreate(any(FactoryDto.class));
        ...
        final FactoryDto responseFactory = getFromResponse(response, FactoryDto.class);
        assertEquals(responseFactory.withLinks(emptyList()), asDto(update));
    }

    @Test
    public void shouldThrowNotFoundExceptionWhenNoFactoryByGivenId() throws Exception {
        final Factory factory = createFactory();
        final String errMessage = "Factory with id " + FACTORY_ID + " not found";
        doThrow(new NotFoundException(errMessage)).when(factoryManager).updateFactory(any());
    ...
    @Test
    public void shouldThrowBadRequestExceptionWhenFactoryUpdateNull() throws Exception {
        final Response response = given().auth()
                                         .basic(ADMIN_USER_NAME, ADMIN_USER_PASSWORD)
                                         .contentType(APPLICATION_JSON)
                                         .when()
    ...
    @Test
    public void shouldReturnDefaultFactorySnippetWhenTypeNotSpecified(ITestContext context) throws Exception {
        final String urlFactorySnippet = getServerUrl(context) + "/factory?id=" + FACTORY_ID;
        when(factoryManager.getFactorySnippet(eq(FACTORY_ID), anyString(), any(UriInfo.class))).thenReturn(urlFactorySnippet);

        given().expect()
               .statusCode(200)
               .body(equalTo(urlFactorySnippet))
    ...
    @Test
    public void shouldReturnFactorySnippetWhenSnippetTypeSpecified(ITestContext context) throws Exception {
        final String htmlFactorySnippet = "<script type=\"text/javascript\" src=\"" + getServerUrl(context) +
                                         "/factory/resources/factory.js?" + FACTORY_ID + "\"></script>";
        when(factoryManager.getFactorySnippet(eq(FACTORY_ID), anyString(), any(UriInfo.class))).thenReturn(htmlFactorySnippet);

        given().expect()
               .statusCode(200)
               .contentType(TEXT_PLAIN)
               .body(equalTo(htmlFactorySnippet))
               .when()
               .get(SERVICE_PATH + "/" + FACTORY_ID + "/snippet?type=html");
    }

    @Test
    public void shouldThrowBadRequestExceptionWhenSnippetTypeIsUnsupported() throws Exception {
        when(factoryManager.getFactorySnippet(eq(FACTORY_ID), anyString(), any(UriInfo.class))).thenReturn(null);

        final Response response = given().expect()
                                         .statusCode(400)
                                         .contentType(APPLICATION_JSON)
                                         .when()
                                         .get(SERVICE_PATH + "/" + FACTORY_ID + "/snippet?type=xml");

        final ServiceError err = getFromResponse(response, ServiceError.class);
        assertEquals(err.getMessage(), "Snippet type \"xml\" is unsupported");
    }
//
//    @Test
...
//    }
//
    private static String getServerUrl(ITestContext context) {
        final String serverPort = String.valueOf(context.getAttribute(EverrestJetty.JETTY_PORT));
